...
class Edrone():
	...
	def __init__(self):
		...
		self.Kp = [832* 0.06,832* 0.06,1662* 0.06]
		self.Ki = [0,0.000000001,0.985* 0.008]
		self.Kd = [5000* 0.3,5000* 0.3,5000* 0.3]
	...
	#	8. Add error_sum
		...
		self.cmd.rcThrottle = int(1500 + self.error[2]*self.Kp[2] + self.differential_error[2]*self.Kd[2] - self.sum_error[2]*self.Ki[2])
...
if __name__ == '__main__':
	e_drone = Edrone()
	plst= [[0,0,0],[0,0,23],[2,0,23],[2,2,23],[-2,2,23],[-2,-2,23],[2,-2,23],[2,0,23],[0,0,23]]
	...
	for i in range(1,len(plst)):
		e_drone.drone_position = plst[i-1]
		e_drone.setpoint = plst[i]
